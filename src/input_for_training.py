...
def create_codebook(images):
    ...
    kmeans = cluster.KMeans(n_clusters = codebook_words, n_init=25)
    kmeans.fit(features)
    codebook = kmeans.cluster_centers_
    print("Codebook generated...")
    np.save('codebook_combined.npy',codebook)
...
training_input = np.array(extract_input_features(train_images))
np.save('training_input.npy',training_input)

testing_input = np.array(extract_input_features(test_images))
np.save('testing_input.npy',testing_input)
